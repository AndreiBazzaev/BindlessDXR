...
void ClosestHit(inout HitInfo payload, Attributes attrib)
{
	...
	uint frameIndex = frameIndexBuffer[0];
	uint2 dimentions = DispatchRaysDimensions().xy;
	uint pixelID = dimentions.x * DispatchRaysIndex().y + DispatchRaysIndex().x;
	uint seed = GetWangHashSeed(pixelID * frameIndex);
	float randomVal = rand(seed);
	payload.colorAndDistance = float4(float3(randomVal, randomVal, randomVal), RayTCurrent());
		//
